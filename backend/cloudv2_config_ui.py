...
from backend.cloudv2_config import get_config_file_path, load_editable_config, normalize_config, save_config
...
class ConfigUI:
    ...
    def _start_monitoring(self):
        ...
        script_candidates = [
            os.path.join(project_dir, "backend", "run_monitor.py"),
            os.path.join(project_dir, "cloudv2-ping-monitoring.py"),
        ]
        script_path = None
        for candidate in script_candidates:
            if os.path.exists(candidate):
                ...
                break
        pid_file_path = os.path.join(project_dir, ".cloudv2-monitor.pid")
        if not script_path:
            messagebox.showerror("Erro", "Arquivo de entrada do monitor nao encontrado.")
            return
